function gdf = clean_detector_test(values,fs)
...
lpf1 = 70; % low pass filter for spikey component
hpf  = 20; % high pass filter for spikey component

%% Initialize things
all_spikes  = [];
nchs = size(values,2);
tmp_spikes = cell(nchs,1);
...
parfor j = 1:nchs
    ...
    tmp_spikes{j} = [repmat(j,size(out,1),1) out]
   
end
for j = 1:nchs
all_spikes = [all_spikes;tmp_spikes{j}];
end
gdf = all_spikes;
